...
public class Trajectory : MonoBehaviour
{
    ...
    private float previousJulianDate;
    private bool isTrajectoryVisible = false;  // 궤적이 보이는 상태인지 추적
    ...
    void Start()
    {
        ...
        {
            previousJulianDate = TimeManager.Instance.GetJulianDate();
        }
    }

    void Update()
    {
        if (TimeManager.Instance == null || !isTrajectoryVisible) return;

        // TimeManager의 시간이 변경되었는지 확인
        float currentJulianDate = TimeManager.Instance.GetJulianDate();
        if (currentJulianDate != previousJulianDate)
        {
            // 시간이 변경되었으므로 궤적 다시 계산
            CalculateAndDrawTrajectories();
            previousJulianDate = currentJulianDate;
        }
    }
    ...
    public void CalculateAndDrawTrajectories()
    {
        ...
        isTrajectoryVisible = true;  // 궤적이 그려졌으므로 상태 업데이트
    }
    ...
    public void ClearExistingTrajectories()
    {
        ...
        isTrajectoryVisible = false;  // 궤적이 지워졌으므로 상태 업데이트
    }
    ...
    void CalculateStarTrajectories()
    {
        ...
        foreach (var starData in starDataDict.Values)
        {
            ...
            // 현재 시간을 기준으로 궤적 계산
            DateTime currentTime = TimeManager.Instance.GetCurrentDateTime();

            for (int i = 0; i <= arcSegments; i++)
            {
                // 시간 비율 계산 (0~1 사이의 값)
                float timeRatio = (float)i / arcSegments;
                // 실제 시간 계산 (현재 시간부터 durationHours 후까지)
                DateTime trajectoryTime = currentTime.AddHours(timeRatio * durationHours);
