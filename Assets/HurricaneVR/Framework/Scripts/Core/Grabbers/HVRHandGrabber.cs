...
namespace HurricaneVR.Framework.Core.Grabbers
{
    public class HVRHandGrabber : HVRGrabberBase
    {
        ...
        [Tooltip("The amount of time after a grab that the hand will re-enable collisions")]
        public float HandGrabCollisionTimeout = 1f;
        ...
        [Tooltip("The physics hand chases this target.")]
        public Transform PhysicsHandTarget;
        ...
        public Transform ControllerHandTarget => PhysicsHandTarget ? PhysicsHandTarget : TrackedController;
        ...
        private bool _checkingEnableCollision;
        private float _collisionDisableTime;
        ...
        protected virtual void UpdatePushing()
        {
            if (_pushing)
            {
                ...
                if (Vector3.SqrMagnitude(Pusher.size - _pusherSize) < .0001f)
                {
        ...
        private void UpdatePostMoveGrab()
        {
            ...
            {
                if ((Time.time - _collisionDisableTime > HandGrabCollisionTimeout) || Vector3.SqrMagnitude(transform.position - ControllerHandTarget.transform.position) < .0025f)
                {
        ...
        protected override void CheckSwapRelease(HVRGrabbable grabbable)
        {
            ...
            if (grabbable.PrimaryGrabber && grabbable.PrimaryGrabber.IsDistanceGrabber)
            {
                grabbable.SwappingDistanceToHand = true;
            }
        ...
        private IEnumerator MoveGrab()
        {
            ...
            if (!GrabbedTarget.DisableHandCollision)
            {
                ...
                _collisionDisableTime = Time.time;
            }
